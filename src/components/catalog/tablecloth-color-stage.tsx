...
// Draws the swatch photo once, scaled/cropped to fully cover the given
// frame (like CSS `object-fit: cover`) — centered, aspect-preserved, no
// tiling. A repeating tile always showed a seam somewhere (mismatched
// phase on patterns, a stamped-grid look on solids); one un-repeated copy
// has no seam to get wrong, at the cost of not showing the fabric's repeat
// at true physical scale.
function drawSwatchCover(img: HTMLImageElement, width: number, height: number): HTMLCanvasElement {
...
export function TableclothColorStage({
  ...
  const swatchCoverRef = useRef<HTMLCanvasElement | null>(null);
  ...
  const paint = () => {
    ...
    if (swatchCoverRef.current) {
      ctx.drawImage(swatchCoverRef.current, 0, 0, width, height);
    } else {
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      const canvas = canvasRef.current;
      if (!canvas) return;
      swatchCoverRef.current = drawSwatchCover(img, canvas.width, canvas.height);
      paint();
    };
    img.onerror = () => paint();
    // eslint-disable-next-line react-hooks/exhaustive-deps
